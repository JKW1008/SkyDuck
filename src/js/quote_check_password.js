...
document.addEventListener("DOMContentLoaded", () => {
    ...
    password_check_submit.addEventListener("click", () => {
        ...
        f.append("mode", "quote_password_chk");

        const xhr = new XMLHttpRequest();
        xhr.open("POST", "./pg/quote_board_process.php", true);
        xhr.send(f);
            ...
            if (xhr.status == 200) {
                ...
                try {
                    ...
                    if (data.result == 'success') {
                        ...
                        Swal.fire({
                            title: '비밀번호확인',
                            html: '비밀번호확인이 성공적으로 되었습니다.',
                            icon: 'success',
                            confirmButtonText: '확인'
                        }).then((result) => {
                            self.location.href = "./quote_detail_view.php?idx=" + params["idx"];
                        });
