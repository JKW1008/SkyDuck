document.addEventListener("DOMContentLoaded", () => {
    ...
    const qna_title = document.querySelector("#quote_title");
    const qna_password = document.querySelector("#quote_password");
    ...
    qna_submit.addEventListener("click", () => {
        if (quote_title.value == "") {
            alert("제목을 입력해주세요");
            quote_title.focus();
            return false;
        };

        if (quote_password.value == "") {
            alert("비밀번호를 입력해주세요");
            quote_password.focus();
            return false;
        };
        ...
        f.append("title", quote_title.value);
        f.append("password", quote_password.value);
        ...
        xhr.open("POST", "./pg/quote_process.php", true);
        xhr.send(f);
            ...
            if (xhr.status == 200) {
                ...
                try {
                    ...
                    if (data.result === 'success') {
                        ...
                        Swal.fire({
                            title: '문의요청접수',
                            html: '견적문의 요청이 접수 되었습니다.',
                            icon: 'success',
                            confirmButtonText: '확인'
                        }).then((result) => {
                            self.location.href = "./quote.php";
                        });
                    };
